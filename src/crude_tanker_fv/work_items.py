...
def item(
    ...
    **extra,
    ...
    return dict(
        ...
        blocking_decision_ids=blockers or [],
...
def validate(doc):
    ...
    for row in doc["items"]:
        ...
        if not isinstance(row["evidence"], list) or not isinstance(
            row["blocking_decision_ids"], list
        ):
...
def reference(root, ref):
    ...
    if ref.get("runtime"):
        ...
        raw = subprocess.check_output(
            ["git", "show", "HEAD:" + path], cwd=base, stderr=subprocess.DEVNULL
        )
...
def repair(identity, reason):
    return item(
        ...
        "producer",
        "ready",
        "Repair workflow evidence: " + reason,
        waiting="until valid evidence is recorded",
        details=reason,
        authority="repair",
    )
...
def project(root=ROOT, today=None):
    ...
    try:
        ...
        return {
            ...
            "complete": False,
            "items": [
                item("registry:unknown", "producer", "unknown", reason, waiting="repair registry"),
                repair("registry", reason),
            ],
        }
    ...
    def add(row):
        row["authority"] = row.get("authority", "adapter")
        rows[row["id"]] = row
    ...
    def failed(identity, exc):
        add(
            item(
                ...
                "producer",
                "unknown",
                "Inspect unavailable authority: " + str(exc),
                waiting="authority repaired",
            )
        )
    ...
    try:
        ...
        add(
            item(
                "filings:pending",
                "producer",
                "ready" if q["pending_total"] else "done",
                "Drain all pending accessions; commit dispositions before acknowledging",
                evidence=[
                    {"project": "producer", "path": "state/edgar_manifest.jsonl", "runtime": True}
                ],
                waiting="all accessions dispositioned",
                pending_total=q["pending_total"],
                oldest_arrival=q["oldest_arrival"],
                invalid=q["invalid"],
            )
        )
        if q["invalid"]:
            failed("filings:invalid", str(q["invalid"]))
    except (OSError, ValueError, KeyError) as exc:
        failed("filings:pending", exc)
    for filename in ("forks.yaml", "reweight_triggers.yaml"):
        try:
            data = yaml.safe_load(reference(root, {"path": "inputs/" + filename}))
            cards = (
                data["forks"]
                if filename == "forks.yaml"
                else [dict(v, id=k) for k, v in data.items()]
            )
            for card in cards:
                identity = ("fork:" if filename == "forks.yaml" else "trigger:") + card["id"]
                status = card["status"]
                due = str(card.get("execute_after") or card.get("due") or "") or None
                ...
                resolved = (
                    ...
                    if done
                        ...
                        "blocked"
                        if resolver == "owner"
                        else ("ready" if due and due <= today.isoformat() else "waiting")
                    )
                )
                add(
                    item(
                        ...
                        "producer",
                        resolved,
                        card.get("action") or card.get("recommendation") or "Review trigger",
                        resolver,
                        evidence=[{"project": "producer", "path": "inputs/" + filename}],
                        due=due,
                        waiting="registered condition or ruling",
        ...
        for event in seam["events"]:
            ...
            add(
                item(
                    ...
                    "governor",
                    "blocked",
                    (event.get("detail") or event["code"])
                    + "; "
                    + (event.get("action") or "Preserve binding restriction"),
                    "owner",
                    evidence=[
                        {
                            "project": "governor",
                            "path": "monitor/state/seam_latest.json",
                            "runtime": True,
                        }
                    ],
                    waiting="explicit valuation mini-review or gate disposition",
                    blockers=[identity],
                    details=event,
                )
            )
    except (OSError, ValueError, KeyError) as exc:
        failed("governor:consumption", exc)
    ...
    for ticker, path in latest.items():
        if rows.get("roadmap:ten-q2", {}).get("status") == "done" and ticker == "ten":
            continue
        try:
            side = path.with_suffix(".json")
            shadow = json.loads(reference(root, {"path": str(side.relative_to(root))}))
            if shadow.get("version") != 1 or not isinstance(shadow.get("blockers"), list):
                raise ValueError("invalid shadow blocker contract")
            reference(root, shadow["report_evidence"])
            for n, block in enumerate(shadow["blockers"]):
                if (
                    block["resolver"] not in ("owner", "agent", "external")
                    or not block["next_action"]
                ):
                    raise ValueError("invalid shadow resolver")
                add(
                    item(
                        "shadow:" + ticker + ":" + str(block["id"]),
                        "producer",
                        "blocked",
                        block["next_action"],
                        block["resolver"],
                        evidence=[{"project": "producer", "path": str(side.relative_to(root))}],
                        waiting=block["condition"],
...
def queues(doc):
    def line(r):
        return f"{r['id']} [{r['status']}; resolver {r['resolver']}] — {r['next_action']}" + (
            f"; blockers: {', '.join(r['blocking_decision_ids'])}"
            if r["blocking_decision_ids"]
            else ""
        )
...
def record_shadow(report, blockers, root=ROOT):
    path = Path(report)
    if path.parent != Path("decisions") or not re.fullmatch(
        r"[a-z0-9]+_shadow_build_\d{4}-\d{2}-\d{2}\.md", path.name
    ):
        raise ValueError("shadow report must be a scoped dated decisions path")
    for block in blockers:
        if not {"id", "resolver", "next_action", "condition", "blocking_decision_ids"} <= set(
            block
        ) or block["resolver"] not in ("owner", "agent", "external"):
...
def receipts(root=ROOT):
    lines = []
    patterns = [
        ("producer execution", root, "state/operations/runs/*.json"),
    ...
    for title, base, pattern in patterns:
        ...
        if not paths:
            lines.append(title + ": UNKNOWN — missing receipts")
            continue
        try:
            ...
            lines.append(title + ": UNKNOWN — " + str(exc))
    for title, base, path in [
        ("accepted publication", root, "state/publications/current.json"),
        ("consumption", root.parent / "portfolio-governance", "monitor/state/seam_latest.json"),
    ]:
        try:
            r = json.loads((base / path).read_text())
            lines.append(title + ": " + str(r.get("publication_id", r.get("id", "UNKNOWN"))))
        except (OSError, ValueError) as exc:
            lines.append(title + ": UNKNOWN — " + str(exc))
    for title, base in [
        ("producer", root / "state"),
        ("governor", root.parent / "portfolio-governance/monitor/state"),
    ]:
        ...
        try:
            ...
            lines.append(
                ...
                + " delivery: "
                + str(len(p))
                + " unresolved exact-message receipt(s); SMTP accepted is not proof of inbox delivery"
            )
        except (OSError, ValueError) as exc:
            lines.append(title + " delivery: UNKNOWN — " + str(exc))
    lines.append(
...
def record(row, root=ROOT):
    ...
    if not row["evidence"]:
        raise ValueError("cited evidence required")
    for ref in row["evidence"]:
    ...
    with locked(root / "state/work_items.lock"):
        ...
        validate(doc)
        atomic_json(root / "work_items.yaml", doc)
        commit_paths(root, ["work_items.yaml"], "chore(workflow): record " + row["id"])


def main(argv=None):
    ap = argparse.ArgumentParser(description=__doc__)
    ap.add_argument("command", choices=["show", "sync", "shadow", "record"])
    ...
    if args.command == "shadow":
        record_shadow(args.report, json.load(sys.stdin))
        return 0
