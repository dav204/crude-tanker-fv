...
def normalize(raw, printed):
    ...
    if m:
        ...
        year=int(m[2])+(2000 if len(m[2])==2 else 0) if m[2] else printed.year+(month<printed.month)
    ...
    if m:
        q=int(m[1]);year=int(m[2])+(2000 if len(m[2])==2 else 0) if m[2] else printed.year+(q<(printed.month-1)//3+1)
...
def construct_panel(iso, panel, deltas):
    printed=date.fromisoformat(iso);start=quarter(printed)
    quotes={}; originals={}
    for raw,value in panel.items():
        ...
        quotes[kind,period]=int(value);originals.setdefault((kind,period),[]).append(raw)
    ...
    if start not in rates:
        required=['%04d-%02d' % (printed.year,m) for m in range(printed.month,((printed.month-1)//3+1)*3+1)]
    ...
    for n,q in enumerate(unknown):
        rates[q]=base+(n<rem)
        provenance[q]={'kind':'derived','method':'calendar-year residual; remainder to earliest unquoted quarters','calendar_year':year,'calendar_rate':annuals[year],'original_labels':originals['year',str(year)],'source_date':iso}
    tail=index(yearkeys[-1])
    for n,delta in enumerate(deltas,1):
        q=label(tail+n);prev=label(tail+n-1)
        if q not in rates:
            rates[q]=rates[prev]+delta
            provenance[q]={'kind':'derived','method':'ruled tail step','delta':delta,'from':prev,'source_date':iso}
    ...
    for q in keys(start,max(8,tail+3-index(start))):
        if q not in rates:
            ...
            rates[q]=rates[prev]
            provenance[q]={'kind':'derived','method':'flat carry-forward','from':prev,'source_date':iso}
    if any(v<=0 for v in rates.values()):
        raise ValueError('invalid nonpositive constructed rate')
    return {'periods':periods,'values':[rates[q] for q in periods],
            'nodes':[dict(provenance[q],period=q,rate=rates[q]) for q in periods],
            'quotes':[{'kind':k,'period':p,'rate':v,'original_labels':originals[k,p],'source_date':iso} for (k,p),v in quotes.items()],
            'proxy_quarters':ordered,'proxy':(sum(quarters[q] for q in ordered)+1)//2}
...
def align(inputs, inputs_dir, valuation_date=None, enabled=None):
    ...
    day=date.fromisoformat(str(valuation_date or VALUATION_DATE or os.environ.get("CRUDE_FV_VALUATION_DATE") or run_timestamp()[:10]))
    start=quarter(day)
    ...
    for cls,values in inputs.market_data.ffa_forward_curve.items():
        ...
        for q in keys(start,horizon):
            if q in mapping:
                ...
                node={'period':q,'rate':values[-1],'kind':'derived','method':'flat carry-forward','from':periods[-1],'source_date':source[-1].get('source_date')}
            else:
                raise ValueError('missing FFA calendar period '+cls+' '+q)
            projected.append(node)
        curves[cls]=[r['rate'] for r in projected];nodes[cls]=projected
    ...
    fleet={};coverage={};extensions=[]
    for cls,values in inputs.fleet.fleet_schedule.items():
        horizon=10 if cls.startswith('Ctr-') else 8
        static=sum(v.count for v in inputs.fleet.vessels if v.cls==cls)
        fleet[cls]=[values[q] if q<len(values) else static for q in range(elapsed,elapsed+horizon)]
        if elapsed+horizon>len(values): extensions.append({'class':cls,'schedule':'fleet','method':'existing static manifest extension'})
    for cls,values in inputs.fleet.coverage_schedule.items():
        horizon=10 if cls.startswith('Ctr-') else 8
        coverage[cls]=[values[q] if q<len(values) else values[-1] for q in range(elapsed,elapsed+horizon)]
...
def align_scenarios(doc, timeline, classes):
    if not timeline:
        return doc
    doc=copy.deepcopy(doc);horizon=int(doc.get('strip_horizon',8))
    for name,scenario in doc['scenarios'].items():
        for cls in classes:
            block=scenario[cls]
            dated={index('%s-Q%s'%(k[3:],k[1])):k for k in block if re.fullmatch(r'q[1-4]_\d{4}',k)}
            if not dated:
            ...
            for q in keys(timeline['projection_start_quarter'],horizon):
                ...
                if index(q)<=last:
                    raise ValueError('missing internal scenario calendar period '+name+' '+cls+' '+q)
                block[key]=list(block[dated[last]])
                node={'scenario':name,'class':cls,'period':q,'kind':'derived','method':'flat carry-forward','from':label(last)}
                if node not in timeline['scenario_extensions']: timeline['scenario_extensions'].append(node)
    return doc
