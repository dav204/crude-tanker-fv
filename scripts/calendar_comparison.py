...
def measure():
    watch = load_watchlist(INPUTS_DIR, live_prices=True)
    docs = _load_all_sectors(INPUTS_DIR)
    rows = []
    from crude_tanker_fv.ffa_promote import construct

    source = json.loads((INPUTS_DIR.parent / "state/ffa_ocr_curves.json").read_text())[
        "2026-09-21"
    ]["curves"]
    october = construct(
        "2026-10-01",
    ...
    for ticker, entry in watch.items():
        ...
        def value(ci, scenarios):
            r, _, _ = _run_scenarios_for_ticker(
                ticker, ci, entry["current_price"], entry["analyst_target"], scenarios, watch
            )
            return {
                "fv": r.probability_weighted_fv,
                "position": r.position_recommendation,
                "nav": compute_nav(ci).nav_per_share,
                "fv_low": min(s.fair_value for s in r.scenarios),
                "fv_high": max(s.fair_value for s in r.scenarios),
                "sleeves": r.sleeve_fvs,
            }
        ...
        for stamp in ("2026-09-22", "2026-10-01", "2027-01-01"):
            ...
            if stamp == "2026-10-01":
                md = replace(
                    aligned.market_data,
                    ffa_forward_curve=dict(
                        aligned.market_data.ffa_forward_curve, **october["curves"]
                    ),
                    twelve_month_tc=dict(
                        aligned.market_data.twelve_month_tc, **october["twelve_month"]
                    ),
                )
            ...
            for doc in short.values():
                doc["strip_horizon"] = int(doc.get("strip_horizon", 8)) - elapsed
            without_extension = value(aligned, short) if elapsed else result
            rows.append(
                {
                    "ticker": ticker,
                    "valuation_date": stamp,
                    "price": entry["current_price"],
                    "baseline": base,
                    "aligned": result,
                    "construction_effect_september": 0.0,
                    ...
                    "total_fv_delta": result["fv"] - base["fv"],
                    "october_construction_delta": constructed["fv"] - result["fv"]
                    if constructed
                    else None,
...
if __name__ == "__main__":
    rows = measure()
    out = Path("outputs/calendar_comparison_2026-09-22.json")
    ...
    for r in rows:
        lines.append(
            ...
            % (
                ...
                r["aligned"]["fv"],
                r["total_fv_delta"],
                r["timeline_to_old_terminal_delta"],
                r["tail_and_terminal_extension_delta"],
                r["baseline"]["position"],
                r["aligned"]["position"],
            )
        )
    lines += [
        "",
        "October construction-only replay relabels the September source month columns to October/November while preserving quoted Q4, Q1 and Cal27 rates. These are synthetic labels, not observed October data. All 25 construction-only deltas: "
        + str(
            {
                r["ticker"]: round(r["october_construction_delta"], 8)
                for r in rows
                if r["october_construction_delta"] is not None
            }
        ),
    ]
    ...
    print(
        "comparison:",
        len(rows),
        "name/date rows; September maximum delta",
        max(abs(r["total_fv_delta"]) for r in rows if r["valuation_date"] == "2026-09-22"),
    )
