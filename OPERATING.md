...
| `delivery-worker` | launchd | every five minutes and on load; catches up after sleep | `work_items.yaml`, `inputs/forks.yaml`, `inputs/reweight_triggers.yaml`, `decisions/*_shadow_build_*.json`, `governance:reviews/completed_quarters.json`, `state/publications/**` … | `work_items.yaml`, `state/work_items_observed.json`, `state/publications/**`, `state/delivery/**`, `state/operations/**`, `state/heartbeat/delivery-worker` … | governor-seam |
