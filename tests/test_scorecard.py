...
def test_write_scorecard_emits_consolidated_verdict_when_valuation_present(tmp_path, rows):
    ...
    assert "validated-and-actionable-long surface is **1 (SB" in text   # 2 -> 1 at the 2026-08-13 rebase: SBLK fell to GOVERNED-WIDE — the PRICE leg (25.2 -> 28.6, NAV unchanged) pushed pnav_mkt over the cheap|fair boundary on the hist basis, so the justified-pnav read flips (consensus_pnav is not a tier input)
