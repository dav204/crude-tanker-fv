...
@pytest.mark.parametrize(
    "stamp,panel,start,front,proxy",
    [
        (
            "2026-09-21",
            {"sep": 10000, "oct": 99999, "q4": 12000, "q1": 11000, "cal27": 10000},
            "2026-Q3",
            10000,
            11500,
        ),
        (
            "2026-10-05",
            {"oct": 99999, "nov": 99999, "q4": 12000, "q1": 11000, "cal27": 10000},
            "2026-Q4",
            12000,
            11500,
        ),
        (
            "2026-11-05",
            {"nov": 10000, "dec": 12000, "q1": 11000, "q2": 10000, "cal27": 10000},
            "2026-Q4",
            11000,
            10500,
        ),
        (
            "2026-12-05",
            {"dec": 10000, "jan": 99999, "q1": 11000, "q2": 10000, "cal27": 10000},
            "2026-Q4",
            10000,
            10500,
        ),
        (
            "2026-12-28",
            {"dec": 10000, "jan": 99999, "q1": 11000, "q2": 10000, "cal28": 10000},
            "2026-Q4",
            10000,
            10500,
        ),
        (
            "2027-01-05",
            {"jan": 99999, "feb": 99999, "q1": 12000, "q2": 11000, "cal28": 10000},
            "2027-Q1",
            12000,
            11500,
        ),
...
def test_dated_panels(stamp, panel, start, front, proxy):
    ...
    assert (
        result["periods"][0] == start and result["values"][0] == front and result["proxy"] == proxy
    )
    ...
    for quote in result["quotes"]:
        ...
        if quote["kind"] == "year":
            ...
            if all(p in mapped for p in periods):
                assert sum(mapped[p]["rate"] for p in periods) == 4 * quote["rate"]
    assert all(
        n["kind"] in ("derived", "quoted") and n["source_date"] == stamp for n in result["nodes"]
    )
...
def test_bad_panels_freeze(panel):
    with pytest.raises(ValueError):
        cal.construct_panel("2026-09-21", panel, (-500, -500))
...
def test_ambiguous_years_and_rollover():
    ...
    with pytest.raises(ValueError):
        cal.normalize("Feb", date(2026, 3, 1))
...
def test_alignment_all_sectors_and_hybrids():
    watch = load_watchlist(INPUTS_DIR)
    ...
    for ticker, entry in watch.items():
        ...
        for cls, values in original.fleet.coverage_schedule.items():
            assert shifted.fleet.coverage_schedule[cls][0] == values[1]
        for cls, values in original.fleet.fleet_schedule.items():
            assert shifted.fleet.fleet_schedule[cls][0] == values[1]
        shifted, _ = _maybe_apply_transactions(shifted, INPUTS_DIR, True)
        result, _, _ = _run_scenarios_for_ticker(
            ticker, shifted, entry["current_price"], entry["analyst_target"], docs, watch
        )
...
def test_missing_scenario_period_blocks():
    ...
    with pytest.raises(ValueError, match="internal scenario"):
        cal.align_scenarios(
            doc, {"projection_start_quarter": "2026-Q4", "scenario_extensions": []}, {"a"}
        )


def test_explicit_ocr_years_are_retained():
