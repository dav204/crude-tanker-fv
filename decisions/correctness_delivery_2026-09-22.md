...
### Activation and operational receipts

Both production repositories were installed from the isolated commits. All three initial full-suite failures were resolved, including the graph check after worker installation. The final rollout group passed 54 tests; the producer weekly-report group passed nine tests, including same-day immutable report preservation. The governor's full deterministic/fault suite passes, including candidate gates remaining pending after observation until an explicit registry review. Python 3.9 parsing was checked for the new modules.

The worker is installed and loaded as `com.crude-tanker-fv.delivery-worker`, every 300 seconds. The first real publication (`1dae62cb8e980c5eea1de2d49261a592d1fc3da41dfd8b7cb66761e89d038712`) was accepted, consumed and reported in governor commit `5bee732`. Its notification receipt is `monitor/state/delivery/adfc681a914f8a700f1a65b39e7e7170707f68723c9393ba5ce07a1709491a22.json`: SMTP accepted, one attempt. A second native worker invocation left one notification and one send attempt. This proves local publication, governor consumption and SMTP acceptance, not inbox placement. The worker does not mint a weekly research run or refresh its dead-man by merely being alive.

Twelve installed Claude prompts match the staged versions; the weekly prompt also matches monitor/PROMPT.md. All four persistent settings scopes contain the manifest, with prior ask/deny rules preserved. Backups are outside Git at `~/.config/fv-correctness-backups/20260922T141418/`. Claude's app exposed an empty window shell during inspection, so a fresh scheduled Claude run has NOT been verified. The permission-mode reset itself is not claimed fixed; the routine grants work independently of that UI selection, subject to any connector that explicitly requires interaction.

The full queue was backfilled to `state/operations/filing-backfill.json`: 100 pending, oldest July 14, zero invalid records. These have not been falsely acknowledged; the revised triage task must read and disposition them. Owner trade decisions remain unchanged.

Rollback: create `state/operations/disabled` to pause the worker, optionally unload its launchd label, and restore the backed-up settings/prompts. Retain `state/publications/`, both delivery queues and governor consumption/run receipts. The accepted-publication pointer advances only after validation; a held run keeps the previous snapshot.
