...
Band unchanged (TRIM/SHORT). CAPT now sits 1.0pp from the HOLD edge at EV -6.0, which sharpens the strobe exposure the work order already flagged for it: read_blocked means no read_flag hysteresis, so the raw label can restate on a small tape move. Evidence for the owner's Phase-4 CAPT disposition; the post-re-expression crude sidecar regenerated by this landing supersedes the 2026-09-01 sidecar as its weight-robustness evidence.
