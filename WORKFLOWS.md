...
- **The declared graph:** `PYTHONPATH=src .venv/bin/python -m crude_tanker_fv.graph check` (R1-R6 over `graph.yaml` — run it after adding or changing any cron job, wrapper lane or scheduled task) and `… graph render --write` (re-renders the block in OPERATING.md; `tests/test_graph.py` fails while it is stale). New node = a `graph.yaml` entry with reads / writes / triggers / `commits`. (2026-09-13)
