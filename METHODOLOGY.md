...
2. **Terminal value at quarter 9 = 1.0× NAV** — arguably should be 0.9× NAV to reflect mid-cycle discount, 1.1× if structural undersupply persists, or **cycle-conditional** (peak 0.9× / mid 1.0× / trough 1.1×, matching the §10 philosophy). **Options memo written 2026-06-21 (B6): `outputs/terminal_value_options_memo.md`** — DECISION PENDING owner. **Sweep refreshed 2026-06-21 over the full 19-name watchlist** (`scripts/terminal_value_sensitivity.py` → `outputs/terminal_value_sensitivity.md`; was 12 names at the 2026-06-05 first run): {0.85, 0.9, 1.0, 1.1, 1.15}. FV % range scales with `w_earn` (0.30 peak ⇒ ~3.7-6.8%; 0.60 below-mid ⇒ ~13-14%); 12 of 19 names never flip. **At the literal 0.9×/1.1× priors, 7 names flip (all band-edge):** 0.9× turns DHT BUY→HOLD and ECO HOLD→TRIM/SHORT (peak, more bearish); 1.1× turns ASC + SBLK TRIM/SHORT→HOLD and GNK HOLD→BUY (GNK deal-pinned — discount it). FLNG/STNG flip only at the 0.85/1.15 extremes. CCEC (most sensitive, 14.4%) does NOT flip. **Memo recommendation: ratify 1.0× now (auditable; marks already conservative; `w_earn` already down-weights the strip at peak), with cycle-conditional as the designated successor pending two adoption triggers; reject uniform 0.9× (wrong sign at troughs) and 1.1× (its broker-gap justification violates the no-calibration-to-broker doctrine).** Until the owner locks it, decision logs for the band-edge names (DHT/ECO/ASC/SBLK, FLNG/STNG at extremes) name the 1.0× assumption explicitly.
