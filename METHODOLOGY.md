...
6. **Newbuild commitments treatment** — *resolved (§3.1)*: for material orderbooks, committed newbuilds are valued at delivered market value less remaining commitment (not at sunk cost), since at cost they understate NAV by the embedded value of cheaply-contracted ships in a hot market (decisive for FRO, ~$5.7/sh). **Time-to-delivery PV discount — *resolved 2026-06-22 (BRUT)*:** the delivered value is PV-discounted by `1.11^(−years_to_delivery)` per vessel (`compute_nav`, constant `NEWBUILD_DELIVERY_DISCOUNT_RATE`; the remaining commitment is kept at face — conservative). It was flagged "minor" while undisturbed, but BRUT (a 100%-newbuild vehicle delivering 2026-29) proved it dominant: **undiscounted it read +116% vs broker NAV (SANITY=FAIL); the discount lands it at +30.6% (OK).** `years_to_delivery` defaults to 0 (on the water → factor 1.0), so the discount is **opt-in per manifest** — applied to BRUT only so far; rollout to the other newbuild books (CAPT/FRO/MPCC/GSL/CMDB) is a pending owner decision (it moves their NAVs and needs a re-validation pass). The strip's terminal NAV advances `years_to_delivery` so ships delivered before q9 de-discount and age in.
