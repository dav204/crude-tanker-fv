...
6. **Newbuild commitments treatment** — *resolved (§3.1)*: for material orderbooks, committed newbuilds are valued at delivered market value less remaining commitment (not at sunk cost), since at cost they understate NAV by the embedded value of cheaply-contracted ships in a hot market (decisive for FRO, ~$5.7/sh). **Time-to-delivery PV discount — *resolved 2026-06-22 (BRUT)*:** the delivered value is PV-discounted by `1.11^(−years_to_delivery)` per vessel (`compute_nav`, constant `NEWBUILD_DELIVERY_DISCOUNT_RATE`; the remaining commitment is kept at face — conservative). It was flagged "minor" while undisturbed, but BRUT (a 100%-newbuild vehicle delivering 2026-29) proved it dominant: **undiscounted it read +116% vs broker NAV (SANITY=FAIL); the discount lands it at +30.6% (OK).** `years_to_delivery` defaults to 0 (on the water → factor 1.0), so the discount is **opt-in per manifest**. **Rolled out 2026-06-22** from BRUT to all newbuild books: **CAPT** (12 VLCC + 8 Suez NB → gap −2.6% → **−17.3%**, the material move; NB-heavy, and the tool now haircuts NB timing more than Pareto seems to — a documented divergence, SANITY-OK), **MPCC** (15 NB → NAV $2.27 → $2.02, −9.4pp), **FRO** (9 near-term NB → −1.1pp, negligible). GSL's NB order is post-snapshot and CMDB has none, so both are unaffected. All 20 names SANITY-OK; CAPT/MPCC drift-annotated; the MPCC pin re-pinned. The strip's terminal NAV advances `years_to_delivery` so ships delivered before q9 de-discount and age in.
...
#### Anchor-basis commensurability — three incompatible cycle-anchor bases (B5, added 2026-06 Week 5)

The TC-vs-spot caveat above is one instance of a general rule: **a cycle-position ratio is (forward-12M TC) / cycle-anchor, so the ratio is only numerically comparable across names whose anchors share a *basis*.** As the tool grew across sectors, three different anchor bases entered the book, and they do not compose:

- **`tc_10yr_mean` — TC-anchored 10-year means** (crude, product, LNG). The original basis; values live in `historical_tce_means.yaml` and the `sectors.{crude,product,lng}.cycle_anchors` blocks (e.g. VLCC $40,000, MR $16,000, LNGC $85,000).
- **`archive_22mo_median` — 22-month Pareto archive medians** (dry bulk; §11.7.5). v1 dry-bulk anchors are medians of a 22-month Pareto Shipping Daily window (Aug-2024 → Jun-2026), not a 10-year TC mean — and that window sits in a firm dry-bulk environment, so the anchors are biased *elevated* vs a true long-run mean (Cape $23,650, Pana $11,900, Supra-Ultra $13,930). Refinement to a longer series is a Q3 item.
- **`fy_calendar_avg` — FY2021-2025 calendar averages** (containerships; §11.8.5). The MB Container Weekly's own FY-average table, embedding a full cycle (2021-22 boom, 2023 trough, 2024-25 disruption boom) in a 5-year window — a boom-tilted average, not a 10-year mean (feeder $20,850, intermediate $33,700, large $41,000).

**Why this matters and where it does not.** It does **not** affect any single name's valuation: each name's cycle position is computed against its own sector's anchor on a self-consistent basis, and the valuation core (`nav` / `blend` / `cycle`) is unchanged. It matters only when a **cross-sector view lines cycle positions up side by side** — a name reading "1.4× its anchor" in dry bulk is not on the same scale as one reading "1.1× its anchor" in crude, because the denominators were built differently (a firm-window median vs a full-cycle TC mean vs a boom-tilted calendar average). Comparing those ratios to rank cheapness *across* sectors is the error this caveat forbids — the same discipline as not composing VIE's spot-anchored multipliers with our TC-anchored ratios.

**Machine surface (B5).** Each `cycle_anchors` block in `inputs/scenario_inputs.yaml` carries an `anchor_basis` token (one of the three above). The delta report (`outputs/delta_report.md`) and `reconcile --all` emit a **MIXED-ANCHOR-BASIS** note whenever the names displayed span more than one basis, naming which sectors sit on which basis. `reconcile --sector` and single-name runs use one basis by construction and do not flag. The reconcile gap remains a valid *within-sector* diagnostic; the flag's job is to stop a reader treating a cross-sector cycle-position comparison as commensurable. The bases converge only when a future refit puts every sector on a common long-run-mean basis (the Q3 dry-bulk / container refinement is the path).
