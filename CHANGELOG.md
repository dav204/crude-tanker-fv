...
- **2026-09-20 — a parked scheduled task looked exactly like a healthy one; now it pages (TASK-PARKED), and a cron commit no longer names a product it did not write.** Owner: *"how is this automation supposed to work effectively if you're always prompting me to approve the same damn task every week which I already approved last week"*. He had approved it. **Three mechanisms look like grants and only one is real.** (1) An "always" answer on Write or Edit cannot persist: the auto-approver keeps only allow-shaped rule suggestions, that list is empty for file-mutating tools, so it bails before the store and the store-writer applies the same filter — the click sets a session flag that dies with the run. Across every app log, Write/Edit auto-approved in a scheduled task **zero times ever, any task**. (2) Stored Bash approvals are literal command strings matched by exact equality, so each week's approval is spent on that week's command — the fork executor holds five rules differing only by a date in a filename. (3) `allowed-tools:` in a task's SKILL.md frontmatter is discarded by the scheduler's parser, and updating a task deletes the line. The governance monitor carried nine correct rules there for ten weeks. The only surface that grants is the `.claude/settings.local.json` in scope for the task's **cwd**, which for app tasks is the bare `~/Projects` dir, not the repo. **What it cost:** the 2026-09-18 weekly monitor did its whole job in 4m11s, parked on its first write, and lost the log line, the mail, the dead-man ping and the commit together — one blocked call, five missing artefacts, and its own ping sat downstream of the block so nothing paged. Two more tasks parked the next morning; 27 prompts have gone unanswered since 8/07. **Our half, guarded:** `sentinel.parked_session_flags` raises **TASK-PARKED** for any live session blocked on a permission prompt past 60 minutes — it found all three on its first run, so it is not vacuous (`tests/test_sentinel.py`, 4 cases incl. the pure-mode drop). Machine-local, so pure mode drops it. **Second defect, same family:** `scripts/sentinel_cron.sh` committed a fixed subject naming "weekly report / news digest" whichever landed, so 0d1d11a (9/19) asserted a news digest that does not exist — the news pull had parked. It now names only what is staged. A subject that names an absent product turns a silent loss into a positive false record in git. The fix on the governance side (one `monitor/land.sh` behind one allowlist rule) is in that repo's 17de11b. Permission-file edits stay the owner's: the classifier refuses them from an agent, which is correct — an agent widening its own permissions is exactly what that block exists to prevent.
- **2026-09-18 — PLAN.md rewritten 980 -> 131 lines, and the reason it grew is now guarded.** Owner, after a day of shipping automation: *"the more I ask you to automate the more hand cranked this feels"*, then *"clean up the fucking plan file or just delete it"*. Not deleted — CLAUDE.md routes every new agent through it and two scheduled tasks declare it as a write. Audited instead, five agents over the whole file, every claim checked against git, the forks registry, RATIFY_LOG, the trigger register and the live surface. The first span alone scored 19 DONE against 11 still live; the "Open threads — start here" section a new agent is told to act on was ~95% closed history. **The mechanism, named:** completion is recorded in `inputs/forks.yaml`, `RATIFY_LOG.md` and the trigger register, while PLAN kept the REQUEST and nobody struck it — so the owner's backlog appeared to grow with every automation that landed. Nine dated STATE INSERT blocks stacked newest-on-top, each pointing at the one below, put ~220 lines of superseded state ahead of anything current; regenerated state had been hand-copied in and rotted (the tier snapshot, the opportunity-set counts, a trigger card folded eight days earlier). One claim was wrong in a block written the same day. *Now:* what is owed and nothing else — five owner decisions, eight agent tasks, the defects, the carry notes, the gate — plus a table pointing at the seven registries that are the actual authorities. **This executes F5** (`prune_ledger_2026-09-02.md`), which ruled the 150-line cap on 9/02 and was never run. Guards so it cannot recur: `test_plan_stays_a_handoff_not_an_archive` (line cap, and at most ONE state-insert block) and `test_plan_points_at_the_registries_rather_than_copying_them`. The old file is in git history; nothing was lost.
- **2026-09-18 — WO5/R4 Phase 5 ratified, and the ratify surfaced a latent clock bug that would have frozen the daily lane.** The anchor now stands at the post-disposition surface with one cause naming the work order and decomposing the six moved rows per name (TEN +7.2, TNK +4.3, CAPT +4.0, NAT +3.3, BRUT +3.1, ECO +2.3), baseline and RATIFY_LOG row in ONE commit per the 8/31 half-commit lesson. *The bug:* `outputs/book_scorecard.json`'s `generated_at` and `state/last_run.json`'s `run_at` describe the same run and TWO readers require them equal — annotate's R1 refuses the run outright when they differ, and `ratify_baseline.sh` copies `run_at` into the baseline's `ratified_at`, which annotate's R2 compares against the anchor surface's `generated_at`. They were two independent `datetime.now()` calls, so they agreed only when a run did not straddle a second boundary. The Phase-4 regen straddled one (21:05:23 vs 21:05:24) and the first Phase-5 ratify inherited it; the drift gate did not care because it reads only the date, which is why this had survived every prior ratify on luck. The annotator would have refused from the next moved row onward and frozen auto-land on (a) — the exact loop this session spent the day removing. *Fixed:* both writers take `loaders.run_timestamp()`, memoised per process, with `reset_run_timestamp()` for tests; the skewed baseline was discarded unpushed, the pair regenerated together, and the ratify re-run against the coherent surface (R2 verified to the second). Guard: `test_the_surface_and_the_run_state_share_one_timestamp` pins that neither writer mints its own clock AND that the committed pair on disk agrees. Suite 948 passed, gate 25 rows all stable.
- **2026-09-18 — WO5/R4 Phase 4: the three Stage-A void dispositions ruled, and they do not agree.** CAPT RETIRED, TNK RETIRED to cycle-relabel, BRUT UPHELD — one commit each, on the Phase-3 frozen evidence, with four independent reads taken first and deliberately not briefed with a provisional view. The question that decided all three turned out to be the same one: **what is the registered GROUND, and does the re-expression reach it?** CAPT and TNK each had exactly one ground, the 2026-08-10 deck artifact, which named its own retire condition; that condition landed on 9/18 and is now guard-held, and the artifacts are gone (CAPT reads TRIM/SHORT -6.0, not BUY-ward, TRIM across all 8 weight sets; TNK TRIM -12.9, 7.9pp from the edge, TRIM across all 8). BRUT has TWO grounds and the re-expression reaches only the junior one: its 2026-07-01 untrustworthiness ground is a balance-sheet and solvency ground, and a scenario re-levelling moves no sheet (NAV was exactly 0.00 on all 25 names). Decisive for BRUT: its live 4-hull entity has never filed a balance sheet, and the constructed Q2 sheet was accepted under its own stated premise, 'the read is VOID regardless (R4)'. The re-expression's new evidence argued the same way — it pushed BRUT's raw band across the BUY edge to +5.2 vs +5.0, a ~1-cent margin that a +0.25% NOK/USD move flips, after strobing three times in nine days, at the weight family's MAXIMUM with ev_sign_stable false. *Two guards shipped with the rulings, both for holes nobody had covered:* `test_read_blocked_and_not_unreliable_renders_the_raw_band` (CAPT is the book's first such row and the work order asked for the pin), and `test_a_cycle_relabelled_name_may_not_print_a_raw_buy` — because `_verdict_position` rewrites a relabelled cell unconditionally, so routing TNK to cycle-relabel would otherwise have disarmed the BUY tripwire for every name in that registry. *One test the work order's blast radius missed:* `test_a_relabelled_name_does_not_count_as_buyward` used CAPT as its subject and reads the LIVE registry; re-pointed to MPCC. Surface: CAPT joins the name-specific shorts (10 -> 11), TNK renders 'rich · cycle position', BRUT unchanged. Gate unmoved (registry membership is not a gate input). Suite 947 passed. Phase 5 — the ratify and the push — remains the owner's.
- **2026-09-18 — WO5/R4 Phases 0-3b: the crude deck re-expressed against the live base, and the guard that stops it going stale again.** The crude scenario curves are ABSOLUTE dollars/day (built 2026-05-29, war-re-tilted 07-02); the base they are measured against has been re-anchored repeatedly since and the curves were not. Measured on 9/18 the same leg meant four different things depending on the class — `pre_mou_baseline` priced a 28% forward discount for Aframax, 20% for Suezmax, 5% for VLCC and an 18% PREMIUM for clean LR2. (The 8/10 Stage-A complaint, that the leg read as near-zero de-escalation, had been overtaken: its registered meaning changed to 'the observed state' via the doha restore, C2 and C3, at which base-tracking is correct. The live defect was class inconsistency.) *Fork B:* the three de-escalation legs re-levelled per class to their registered ratios against the FFA reference (pre_mou 1.00, mou_base 0.80, mou_bear 0.60), quarter shape and low/mid/high spread preserved; `escalation` untouched (already above the 1.25 vessel clamp on all four classes); WEIGHTS untouched. *Fork C:* `test_crude_deck_prices_a_real_spread_against_the_base` pins every leg's forward-vs-base ratio per class, a minimum bear-to-observed spread of 0.30, and the ordering — it reds the next time the base walks under the deck, which is the durable artifact of the whole work order. *Phase 3 freeze*, computed off-tree in a worktree before anything landed and reproduced EXACTLY by the production regen: NAV/share 0.00 on all 25 names, all 15 non-crude names 0.00 on FV and EV (the determinant-leg law), 10 crude names +1.47% to +9.09% FV, six rows over the gate's 2pp threshold (BRUT CAPT ECO NAT TEN TNK, all annotated), no rendered band flip, and exactly the two pins the work order predicted (INSW, ECO). *Material finding routed to the owner's Phase 4:* BRUT's RAW band crosses HOLD -> BUY at EV +5.2 against a +5.0 edge — 0.2pp, no hysteresis, masked by its void today. *Routed, not resolved:* after the re-expression 59% of the crude mass sits on a base-tracking leg and 13% on the only real downside, which is a WEIGHT question the work order's own kill-switch fences out of scope. Phase 4 (the three dispositions) and Phase 5 (ratify, push) remain owner acts. `decisions/r4_deck_reexpression_method_2026-09-18.md`.
- **2026-09-18 (same day, second pass) — the first cut of step 0b was under-specified; an adversarial review caught three blockers before it ever ran.** The step claimed to mirror the 11:15 price-leg lane and copied three of its four stages. Fixed: (1) the commit of the annotator's output was gated on "did the regen run" rather than on the tree, so on a FREEZE-without-(e) path `annotate --apply` wrote decision-log entries nothing committed — which freezes auto-land on (c), holds auto-push, starves the night's price refresh and would have laundered the tape into the fork's own sourcing commit at 2f; it now gates on `git status --porcelain -- outputs decisions` exactly as the lane does. (2) The lane's step 4 was missing entirely: `regen.sh` exits 0 on a dead pipeline and a red guard alike, so 0b now verifies `source_commit == HEAD` and re-runs `tests/test_outputs_hygiene.py` before committing, and refuses the whole fork round on a half-built surface. (3) 2e still treated a row the annotator had REFUSED as the fork's own miss, which reproduced the very halt the ruling removed; rows 0b reports as inherited or refused are now explicitly not the fork's. Also: the regen trigger now covers BOTH conjuncts of precondition (e) (a dark-Mac Monday fails on a stale `state/last_run.json`, not on the surface); staging is `git add -u -- outputs "decisions/*_log.md"` because `-- decisions` would sweep any of the ~94 other tracked files there into a `fork-exec` commit and red graph R6 for thirty days; a new RATIFY-OWED guard refuses to absorb any window containing a non-price determinant, honouring Appendix A's reservation of a two-cause ratify to the owner; step 0's restated expectation no longer contradicts 2e; the live-gate exemption is now attributed to the owner's 9/18 word and the reasoning behind it, NOT to §3.1 step 7 / §5, which exempt the same test from a different clause (the red-BASE check); the `annotate` graph node names its second caller; and `decisions/autopilot_authority_2026-09-02.md` carries a dated supersession note so the law the skill cites no longer reads as the opposite of the installed behaviour.
- **2026-09-18 — OWNER RULING: the fork executor absorbs the inherited tape BEFORE it judges its own promote.** Owner, asked what to automate after five executor halts in five days: *"let the executor annotate and land before judging the suite"*. Three of those halts were the same defect — the drift gate measures from the RATIFIED ANCHOR, so tape that landed since it (a vintage the 11:15 lane could not absorb through a chat's dirty tree, a weekend) showed up as UNEXPLAINED rows against a promote that did not cause them, red the suite, and paged. *Landed:* a new step 0b in the executor skill, placed BEFORE the dry FFA promote because the annotator's determinant proof refuses any window in which a curve moved — once the fork lands a curve it is too late. It runs the 11:15 lane's own proven order: regen if the surface is stale, `annotate --apply` (refusals are the design working and are named in the report, never hand-written around), commit, then `promote land` behind its five preconditions. `tests/test_drift_gate.py::test_live_drift_gate_no_unexplained_drift` is now EXEMPT from the executor's suite check, per the 2026-09-02 autopilot ruling §3.1 step 7 / §5, which had never been carried into the skill: step 2e reads the gate directly against the frozen expectation, so the live-gate test only re-asserts the same rows. The ordering law is intact — the fork's OWN move is still compared at 2e before anything of its own is anchored. *The named cost, mitigated:* landing the tape absorbs band flips unread, so `annotate.first_sentence` now carries `band <from> -> <to>` on a flipped row into the ratify cause (guard `test_the_cause_sentence_names_a_band_flip`; TRMD's BUY -> TRIM/SHORT is the live case). Also: step 2f now forbids hand-writing a log entry for any ticker the fork does not name (a hand entry reads to the gate as a HUMAN annotation, so writing one for a tape row would bless an unreviewed move); the tracked and installed skills had drifted on their description line and are converged; `graph.yaml` models auto-land as triggered by the executor as well as the price-leg lane; CLAUDE.md and WORKFLOWS.md no longer say the re-anchor is human-only, which `promote.land` had already made false.
- **2026-09-18 — THREE readers of the determinant window, two definitions: the land lane froze on a shadow-build draft.** `annotate` excluded `*.yaml.draft` and the sp_scan cursor; `promote._surface_matches_head` (precondition (e)) and the cron's regen trigger each carried their own hand-copied pathspec list and did not. So the 9/18 shadow build's two TEN drafts (c506de7) made (e) read "determinants changed since the surface" while the annotator called the same window pure price — the exact disagreement `test_the_determinant_exclusion_list_has_exactly_one_definition` was written to prevent, and missed because it checked only two of the three readers. *Fixed:* `promote.determinant_paths` is THE definition (pathspec excludes plus the non-determinant filter); (e), `annotate.determinant_changes` and the cron all call it, the cron through a new `python -m crude_tanker_fv.promote determinants <from> [<to>]`. (e)'s message now names the offending paths instead of a diffstat line. Guards: the one-definition test now covers the cron and reds if a third copy reappears; `test_drafts_and_the_scan_cursor_are_not_determinants_for_any_reader` pins the filter. Verified on the live tree: (e) went FAIL -> PASS with the drafts still committed.
- **2026-09-18 — The THURSDAY trigger-check DRAFT task (owner: yes to unattended web access).** `scripts/scheduled_tasks/crude-fv-trigger-check-draft.SKILL.md` (tracked source; the owner installs it) drafts the weekly geopolitics check on the card's due day: in-repo broker layer first (Pareto dailies via pdftotext, MB Tanker Weekly, the deck's weighting rule), then a fixed web source list primary-first (centcom.mil / ukmto.org, which 403 to fetchers and are recorded as such; GlobalSecurity OPREP; Critical Threats; outlets that quote CENTCOM and ministers verbatim), the two-leg tables, the escalation question re-run to ONE proposed disposition, a mandatory self-verification pass built from the 9/10 F1-F6 corrections, and a ready-to-paste card comment. It writes exactly `decisions/trigger_check_<card>_<due>.draft.md` and commits it (`commits: self`, prefix `trigger-check draft`, so an untracked draft never freezes the lanes); drafts-only stands — the register, records and forks stay the owner's. graph.yaml node `planned: true` until installed; `sentinel.trigger_flags` names the draft on the TRIGGER-DUE line (after the event head, key unchanged); the page text says to read it first. Web domains are granted in the task's own frontmatter (the fork-executor precedent); the tracked allowlist is untouched. A dry run of the skill in a worktree (31 tool calls, HOLD, matching the 9/17 tripwires) found the cadence error — the card is due on THURSDAYS, a Wednesday task would never draft before the page — and 13 frictions now folded in (Grep not Bash grep, the Pareto table layout, URL slugs for GlobalSecurity/Critical Threats, presstv.co.uk not .ir, a 30-fetch budget, the evidence horizon, contradictions flagged not edited). Also: the draft pointer sits right after the event head so the owner-queue cut keeps it; R5 flags a `planned` node whose task exists; a `trigger_check_draft` duty watches the draft and its recorded successor (duty globs may be a list and resolve from the repo root); `/record-trigger-check <card> <due>` is the chat route the page names; `tests/test_scheduled_task_skills.py` ties each tracked skill to its node and R6 prefix. INSTALLED the same day from the chat via the app's scheduled-tasks tool (cron `0 9 * * 4`, fires ~09:09 local with jitter); the allowed-tools line was pasted into the task frontmatter, `planned` dropped, the duty flipped to scheduled.
- **2026-09-18 — Four small paging leaks closed.** (1) The weekly report's owner queue read a hand-typed tag tuple that had drifted from `inputs/notify.yaml` (it still listed FORK-EXECUTABLE, agent-class since 9/16, and omitted FORK-OPENED, the one with an objection window) and dropped the card name from every line; `weekly_report.owner_tags` now derives the set from the routing table and the line keeps the label. (2) META-MODE suspended every content check on a dirty tree, the trigger register included, so a dirty week silently swallowed the one page that is a dated fact; `sentinel.trigger_flags` now runs in META-MODE too (other content checks stay off; DIRTY-TOO-LONG unchanged). (3) graph R7's indeterminate case (an off-slot launchd run with no sibling since) was a failure, so a Mac nap past the morning slots red the suite the fork executor gates on and halted forks; it is now a NOTE printed by `graph check`, and the FIRST hour-bearing run after a 10-minute wake window decides: on its slot = catch-up, off by the same offset = shift, off by another offset = another nap (two naps in a day are not a shift). The start slack is 20 minutes because on-slot launchd starts run up to 16 minutes late in the log; a replay of all 597 launchd moments since the 08-17 reboot under this rule reds only the reboot's own tail (08-17 until the two Saturday jobs first ran on the new offset on 08-22) and nothing since. Also: META-MODE tolerates an unparseable register (named in the META note, no crash), and the prose that still said FORK-EXECUTABLE pages (OPERATING, PLAN, forks.yaml header, the executor skill) or that every content check is suspended (README, sentinel_cron.sh, graph.yaml) now matches the code. (4) The news-pull command asked the Saturday sweep to re-check due trigger cards in the digest, a duty the installed task never carried; the sentinel pages them, so the line now says so.
- **2026-09-18 — Chip-spawned sessions merge their own PR.** The two chips spawned on 9/17 (R7 launchd catch-up, page_once key) each opened a PR and stopped; both sat until the owner asked ("why make all these cards for other agents to go off and do other stuff if you're not gonna merge it back"). Landing them needed a base re-assert on GitHub (it did not recompute either diff after main was pushed until `gh api -X PATCH … -f base=main`), a CHANGELOG conflict resolve, and a local merge when a fast-forward push to the PR branch was rejected without a reason. Rule (CLAUDE.md, Workflows): every `spawn_task` prompt ends with the merge clause — green suite, push main, merge, confirm MERGED; the full clause lives in the agent memory. To make room, the price-vintage and credentials rules lost their narrative (it is here).
- **2026-09-18 — TRIGGER-DUE page identity is now the EVENT, not a regex over prose.** The 9/17 fix dated the DUE branch only; a FIRED or DEADLINE-BREACHED flag still keyed to the dateless `TRIGGER-DUE {label}:`, byte-identical to the stale keys in `state/sentinel_state.json`, so a firing of the weekly card before mid-November would have ridden the digest, and any re-fire inside 60 days always would. Now `refresh._trigger_event` writes `[sector] DUE|FIRED|BREACHED <date> — …` at the head of every detail that can page, `notify.page_once_key` reads exactly that anchored head (`TRIGGER_EVENT_RE`; an unknown shape keys on its full text, never the bare label), and a FIRED card's identity is its `fired:` date (set when flipping, dropped on re-arm; a dated card without it keys on the due date it fired at). The three legacy dateless keys in state are now unreachable and age out by 2026-11-16. The page text now names the event (DUE: run the check; FIRED: the reweight decision is owed; BREACHED: run the fallback) and the dangling say-agent route is gone. Guards: `test_page_once_keys` builds the flag from the emitter for all three branches and pins the anchored head; `test_page_once_pages_again_on_rearm_and_on_firing` runs page -> re-arm -> fire -> re-fire -> repeat through the sentinel; `test_live_trigger_fired_field_is_set_when_fired_and_dropped_on_rearm` enforces the field discipline on the live register.
