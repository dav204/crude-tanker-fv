...
**NAV alone covers the price.** NAV/share **$8.80** ≥ price **$5.58** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
_Current price $5.58. Cycle weights held at base across the grid._

## Divergence diagnosis

Tool fair value **$9.27** is +66.1% vs the current price ($5.58) and +30.0% vs the analyst target ($7.13). NAV alone covers the price (NAV/sh $8.80 ≥ $5.58); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
