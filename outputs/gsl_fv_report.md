...
**NAV alone covers the price.** NAV/share **$38.59** ≥ price **$37.60** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
_Current price $37.60. Cycle weights held at base across the grid._

## Divergence diagnosis

Tool fair value **$43.00** is +14.4% vs the current price ($37.60) and -17.4% vs the analyst target ($52.04). NAV alone covers the price (NAV/sh $38.59 ≥ $37.60); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
