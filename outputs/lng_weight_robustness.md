...
## FLNG — at price $31.14, target $25.00
...
- HOLD threshold (FV ≥ $29.58, EV ≥ -5%): _not reachable on the [Set B → Set B-revised] line_
- BUY threshold (FV ≥ $32.70, EV ≥ +5%): _not reachable on the [Set B → Set B-revised] line_
...
- alpha for HOLD = **1.94** (must extrapolate 94% beyond Set B-revised)
- alpha for BUY = **3.82**

**Extrapolated weights that would flip FLNG to HOLD (alpha = 1.94):**

| Scenario | Weight at HOLD threshold | vs Set B | vs Set B-revised |
|---|--:|--:|--:|
| tight_resurgence | 0.197 | +0.097 | +0.047 |
| moderate_tightening | 0.344 | +0.194 | +0.094 |
| glut_base | 0.356 | -0.194 | -0.094 |
| glut_intensifies | 0.103 | -0.097 | -0.047 |
| structural_reset | 0.000 | +0.000 | +0.000 |

**Constructive total (tight + moderate + glut_base) at the flip point: 90%**
  (vs Set B: 80%; Set B-revised: 85%). Whether this is defensible depends on whether the Ras Laffan + winter view warrants a constructive environment lasting deep into 2027 with only 10% on the bear cases.

---

## CCEC — at price $22.60, target $25.17
...
- HOLD threshold (FV ≥ $21.47, EV ≥ -5%): _not reachable on the [Set B → Set B-revised] line_
- BUY threshold (FV ≥ $23.73, EV ≥ +5%): _not reachable on the [Set B → Set B-revised] line_
