...
**NAV alone covers the price.** NAV/share **$10.72** ≥ price **$8.75** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
Tool fair value **$10.42** is +19.1% vs the current price ($8.75) and +46.8% vs the analyst target ($7.10). NAV alone covers the price (NAV/sh $10.72 ≥ $8.75); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
