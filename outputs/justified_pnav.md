...
**RONAV_norm is return on *marked NAV*, not on book**, and **through-cycle, not NTM**: `normalized_annual_EPS / NAV/sh`, where the EPS runs the dividend-strip earnings machinery with every vessel class's TCE pinned to its cycle anchor (`historical_tce_means`), NOT the FFA forward curve. Book always 'earns well' mid-cycle and says nothing about whether the market value is justified; the FFA front end is the hot NTM number that would inflate the multiple — both are deliberately avoided.
