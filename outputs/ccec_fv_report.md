...
**NAV alone covers the price.** NAV/share **$28.10** ≥ price **$21.51** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
_Current price $21.51. Cycle weights held at base across the grid._

## Divergence diagnosis

Tool fair value **$22.88** is +6.3% vs the current price ($21.51) and -9.1% vs the analyst target ($25.17). NAV alone covers the price (NAV/sh $28.10 ≥ $21.51); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
