...
**NAV alone covers the price.** NAV/share **$15.49** ≥ price **$12.60** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
_Current price $12.60. Cycle weights held at base across the grid._

## Divergence diagnosis

Tool fair value **$16.03** is +27.2% vs the current price ($12.60) and -15.2% vs the analyst target ($18.90). NAV alone covers the price (NAV/sh $15.49 ≥ $12.60); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
