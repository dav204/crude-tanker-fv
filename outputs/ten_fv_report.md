...
**NAV alone covers the price.** NAV/share **$80.79** ≥ price **$37.14** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
_Current price $37.14. Cycle weights held at base across the grid._

## Divergence diagnosis

Tool fair value **$53.32** is +43.6% vs the current price ($37.14) and +3.5% vs the analyst target ($51.50). NAV alone covers the price (NAV/sh $80.79 ≥ $37.14); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
