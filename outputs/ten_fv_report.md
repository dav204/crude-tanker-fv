...
**NAV alone covers the price.** NAV/share **$88.16** ≥ price **$44.32** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
Tool fair value **$59.21** is +33.6% vs the current price ($44.32) and +15.0% vs the analyst target ($51.50). NAV alone covers the price (NAV/sh $88.16 ≥ $44.32); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
