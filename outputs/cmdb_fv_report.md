...
**NAV alone covers the price.** NAV/share **$32.23** ≥ price **$17.60** at base cycle weighting, so the strip provides no extra hurdle — the implied breakeven floor is effectively zero (rates could fall to ~0 and the price would still be justified by vessel value alone). The market is pricing the fleet at a discount to NAV.
...
Tool fair value **$19.84** is +12.7% vs the current price ($17.60) and -29.1% vs the analyst target ($27.98). NAV alone covers the price (NAV/sh $32.23 ≥ $17.60); the dividend strip provides no extra hurdle, so the implied breakeven floor is effectively zero — the market is pricing the fleet at a discount to vessel value.
